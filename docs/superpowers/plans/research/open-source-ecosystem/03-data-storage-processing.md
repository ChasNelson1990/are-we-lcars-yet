...
1. **llama.cpp** — License: MIT · Status: actively maintained, ships continuous build-tagged releases rather than semantic versions (b9838 as of June 29, 2026); a 2026 CUDA-kernel rewrite substantially improved throughput on Ada Lovelace/Blackwell-class GPUs; 120,898+ GitHub stars as of July 19, 2026 · Verified 2026-07-21: confirmed via the live `ggml-org/llama.cpp` GitHub repo (ongoing commits, current tag) and independent 2026 coverage (tech-insider.org, aithinkerlab.com) of the engine rewrite and star count.
...
2. **Ollama** — License: MIT for the CLI/server (`ollama/ollama`, which wraps a llama.cpp-derived engine); the newer bundled desktop GUI app has a separately-tracked, unresolved licensing gap — GitHub issue #11634 (open as of 2026) notes the desktop app ships with no visible license or EULA distinct from the MIT-licensed CLI it's installed alongside · Status: actively maintained, v0.23.3 (May 13, 2026), with a June 2026 `ollama launch` feature and January 2026 Anthropic-API-format compatibility added; 52 million monthly downloads reported in Q1 2026 · Verified 2026-07-21: confirmed via the live `ollama/ollama` GitHub repo/LICENSE file and the open GitHub issue itself, cross-checked against 2026 adoption coverage (dev.to, angelo-lima.fr).
   The most widely-adopted "consumer GPU or unified-memory mini PC at home" runtime: a single binary that pulls quantized GGUF models and serves them locally with minimal setup, popular specifically on Apple Silicon unified-memory Macs and consumer NVIDIA GPUs. Caveat worth flagging to readers (structurally similar to Open WebUI's license caveat in Task 1's Primary interface row): the CLI/server stays MIT, but the newer desktop app's licensing is unresolved, so "Ollama is fully MIT" is only true of the core engine, not every component Ollama now ships.
...
Start with Ollama for the actual "consumer GPU or unified-memory mini PC at home" case in the claim — it's the lowest-friction way to run a quantized GGUF model locally today, with the caveat that the bundled desktop app's license (unlike the MIT CLI) hasn't been fully clarified by the maintainers, so stick to the CLI/server if license purity matters. Drop to bare llama.cpp instead if that ambiguity is a dealbreaker, or for tighter control over quantization/build flags — it's the actual engine underneath most of these tools anyway. Reach for vLLM only once "one model, one user" stops being the real workload — e.g., serving Home Assistant Assist, Open WebUI, and oterm (all from Task 1) off one shared backend at once.
...
1. **OpenZFS** — License: CDDL (a weak-copyleft OSI-approved license; notably GPL-incompatible per the FSF, which is why it ships as a loadable kernel module on Linux rather than being upstreamed) · Status: actively maintained, OpenZFS 2.4.0 announced with 121 contributors — the largest feature release since 2.0 five years ago, including stabilized RAID-Z expansion (add a single disk to an existing RAID-Z vdev without rebuilding it) and mature dRAID; first full integration ships in TrueNAS 26 "Halfmoon" · Verified 2026-07-21: confirmed via OpenZFS's own live documentation and GitHub releases, plus independent 2026 coverage (FreeBSD Foundation, DATAZONE) of the 2.4 feature set.
...
2. **SnapRAID** — License: GPL-3.0 · Status: actively maintained, SnapRAID CLI v14.8 and the companion SnapRAID Daemon v1.13 both released July 2026 · Verified 2026-07-21: confirmed via the live `amadvance/snapraid` GitHub releases page showing a release within the research month.
